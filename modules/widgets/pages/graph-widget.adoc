...
xref:{page-component-version}@gsql-ref:appendix:interpreted-gsql-limitations.adoc[in which not all GSQL query features are available.]
...
Try following along with the queries in xref:{page-component-version}@gsql-ref:tutorials:gsql-101/index.adoc[] using the same small friendship graph.
Remember that Insights only works with GSQL queries in xref:{page-component-version}@gsql-ref:appendix:interpreted-gsql-limitations.adoc[Interpreted mode.]
